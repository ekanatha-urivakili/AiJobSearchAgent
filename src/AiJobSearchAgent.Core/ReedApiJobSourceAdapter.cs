...
public sealed class ReedApiJobSourceAdapter : IJobSourceAdapter
{
    ...
    private static JobPosting? MapJob(ReedJobDto item, JobSearchCriteria criteria)
    {
        ...
        return new(
            "Reed", jobId, url,
            item.JobTitle, item.EmployerName, item.LocationName,
            item.Distance ?? criteria.RadiusMiles,
            employmentType, workMode,
            salaryMin, salaryMax, dayRateMin, dayRateMax, months,
    ...
    private static EmploymentType InferEmploymentType(ReedJobDto item, string description)
    {
        ...
        if (jobType.Contains("contract") || jobType.Contains("freelance") || jobType.Contains("interim"))
            return EmploymentType.Contract;
        if (jobType.Contains("permanent") || jobType.Contains("full time"))
            return EmploymentType.Permanent;

        var haystack = $"{item.JobTitle} {description}".ToLowerInvariant();
        return haystack.Contains("contract") ? EmploymentType.Contract : EmploymentType.Permanent;
    }
    ...
    private sealed record ReedJobDto(
        int? JobId, string? JobTitle, string? EmployerName, string? LocationName,
        decimal? MinimumSalary, decimal? MaximumSalary, string? Currency,
        string? JobDescription, string? JobUrl, string? JobType,
        double? Distance, string? Date);
}
